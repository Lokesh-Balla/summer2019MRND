...
class RequestGenerator(Thread):
    ...
    def detail_college(self, college_id):
        url = "http://localhost:8080/college/api/" + str(college_id)
        response = requests.get(url, auth=HTTPBasicAuth('lokesh', 'traceon'))
    ...
    def run(self):
        ...
        try:
            ...
            if len(colleges) > 0:
                college_id = randint(96, len(colleges)+96)
    ...
    def list_colleges(self):
        url = "http://localhost:8080/college/api"
        response = requests.get(url, auth=HTTPBasicAuth('lokesh', 'traceon'))
        if response.status_code != 200:
