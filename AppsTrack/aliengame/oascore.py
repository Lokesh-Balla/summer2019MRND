...
class Score(Actor):
    def __init__(self):
        ...
        self.locations = (SCREENRECT.topleft, (SCREENRECT.right-100, 0), (SCREENRECT.right-100, SCREENRECT.bottom-self.rect.bottom ), (0, SCREENRECT.bottom-self.rect.bottom))
        self.coordinates = cycle(self.locations)
        self.pos = [0, SCREENRECT.bottom-self.rect.bottom]
        self.rect = (self.pos[0], self.pos[1], 100, 100)

    def update(self, score):
        self.pos = list(next(self.coordinates))
        self.image = (self.font.render("Score: "+str(score), True, (255, 255, 255)))
        self.rect=(self.pos[0], self.pos[1], 100, 100)
...
class Shot(Actor):
    "The big payload"
    def __init__(self, player, left, right, angle):
        self.image = pygame.transform.rotate(Img.shot, angle)
        self.rect = self.image.get_rect()
    ...
    def update(self):
        ...
        self.rect.right = self.rect.right + self.right
        self.rect.left = self.rect.left - self.left
...
def main():
    ...
    while player.alive or explosions:
        ...
        for actor in [player] + shots + aliens + explosions:
        ...
        if not player.reloading and keystate[K_SPACE] and len(shots) < MAX_SHOTS:
            shots.append(Shot(player, left, right, angle))
        ...
        for shot in shots:
            ...
            if hit != -1:
                score += 2
                score_obj.erase(screen, background)
        ...
        # Draw everybody
        for actor in [player] + aliens + shots + explosions + [score_obj]:
            actor.draw(screen)
