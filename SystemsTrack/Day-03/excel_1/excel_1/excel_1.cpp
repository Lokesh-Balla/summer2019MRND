...
void print_sheet(struct cell **sheet, int has_header){
	int value = 0, count = 0;
	int rows = ROWS;
	int cols = COLS;
	if (has_header == 1){
...
struct cell **SET(struct cell **sheet, int row, int col, int value, char *relation, int has_header){
	if (has_header == 1){
		row = row + 1;
	}
	if (relation == NULL){
...
void EXPORT(struct cell **sheet, char *argument, int has_header){
	FILE *fp;
	int k = 0, flag = 0; 
...
struct cell **IMPORT(struct cell **sheet, char *argument, int has_header){
	FILE *fp;
	fp = fopen(argument, "r");
	int rows = ROWS;
	int cols = COLS;
...
struct cell **parse_command(char *command, struct cell **sheet, char *filename){
	...
	if (strcmp(option, "SET") == 0){
		...
		if (has_realtion != 1){
			...
			sheet = SET(sheet, row, col, value, NULL, has_header);
		}
		else{
			sheet = SET(sheet, row, col, value, relation, has_header);
		}
		free(rc);
	}
		...
		int x = 0, y = 0;
		char *compare_stirng = "header=true";
		char *fheader = (char *)malloc(sizeof(char) * 100);
		while (without_change[i] != '\0'){
			if (without_change[i] == ' '){
				break;
			}
			fheader[i] = without_change[i];
			i++;
		}
		fheader[i] = '\0';
		while (without_change[i] != '\0' || compare_stirng[y] != '\0'){
			if (without_change[i++] == compare_stirng[y++] || without_change[i++] == toupper(compare_stirng[y++])){
				continue;
			}
			else{
				has_header = 0;
		...
		sprintf(fname, "%s", without_change);
		fname = check_and_add_csv(fname);
		sheet = IMPORT(sheet, fname, has_header);
	}
	else if (strcmp(option, "SAVE") == 0){
		if (strcmp(filename,"0000")!=0){
			EXPORT(sheet, filename, has_header);
		}
	}
